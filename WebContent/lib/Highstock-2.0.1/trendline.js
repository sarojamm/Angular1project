        ...
        for (i = 0 ; i < series.length ; i++){
            ...
            if ( s.regression && !s.rendered ) {
                ...
                if (regressionType == "linear") {
                    ...
                    console.error("Invalid regression type: " ,
regressionType) ;
                    break;
                }
                ...
                regression.rSquared =
coefficientOfDetermination(s.data, regression.points).toFixed(2);
                regression.rValue =
Math.sqrt(regression.rSquared,2).toFixed(2) ;
                extraSerie.data = regression.points ;
                extraSerie.name =
extraSerie.name.replace("%r2",regression.rSquared);
                extraSerie.name =
extraSerie.name.replace("%r",regression.rValue);
                extraSerie.name =
extraSerie.name.replace("%eq",regression.string);
    ...
    function _exponential(data) {
        ...
        var A = Math.pow(Math.E, (sum[2] * sum[3] - sum[5] * sum[4]) /
denominator);
        ...
        var string = 'y = ' + Math.round(A*100) / 100 + 'e^(' +
Math.round(B*100) / 100 + 'x)';
     ...
     *              N * Σ(X^2) - Σ(X)^2
     *
     * correlation = N * Σ(XY) - Σ(X) * Σ (Y) / √ (  N * Σ(X^2) - Σ(X)
) * ( N * Σ(Y^2) - Σ(Y)^2 ) ) )
     *
     */
    function _linear(data) {
        ...
        var gradient = (n * sum[3] - sum[0] * sum[1]) / (n * sum[2] -
sum[0] * sum[0]);
        var intercept = (sum[1] / n) - (gradient * sum[0]) / n;
        //var correlation = (n * sum[3] - sum[0] * sum[1]) /
Math.sqrt((n * sum[2] - sum[0] * sum[0]) * (n * sum[4] - sum[1] *
sum[1]));
...
string: string};
    ...
    function _logarithmic(data) {
        ...
        var string = 'y = ' + Math.round(A*100) / 100 + ' + ' +
Math.round(B*100) / 100 + ' ln(x)';
    ...
    function _power(data) {
        ...
        var string = 'y = ' + Math.round(A*100) / 100 + 'x^' +
Math.round(B*100) / 100;
    ...
    function _polynomial(data, order) {
        ...
        var lhs = [], rhs = [], results = [], a = 0, b = 0, i = 0, k =
order + 1;
        ...
        for(var i = equation.length-1; i >= 0; i--){
            if(i > 1) string += Math.round(equation[i]*100) / 100 +
'x^' + i + ' + ';
            else if (i == 1) string += Math.round(equation[i]*100) /
100 + 'x' + ' + ';
    ...
    function _loess (data, bandwidth) {
        ...
        for(var i in xval)
        {
            ...
            if (meanXSquared == meanX * meanX)
                ...
                beta = (meanXY - meanX * meanY) / (meanXSquared -
meanX * meanX);
    ...
    function  gaussianElimination(a, o) {
        var i = 0, j = 0, k = 0, maxrow = 0, tmp = 0, n = a.length -
1, x = new Array(o);
        for (i = 0; i < n; i++) {
           maxrow = i;
           for (j = i + 1; j < n; j++) {
